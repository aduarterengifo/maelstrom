...
type echo_body = { echo : string; msg_id : int }
type generate_body = { msg_id : int }
...
type read_body = { msg_id : int option }
...
let inbound_body_of_yojson (json : Yojson.Safe.t) : inbound_body =
  ...
  | `Broadcast ->
      ...
      let message =
        ...
        | v ->
            failwith ("Unexpected type for message: " ^ Yojson.Safe.to_string v)
      in
...
let yojson_of_inbound_body = function
  ...
  | Generate { msg_id } ->
      `Assoc [ ("type", `String "generate"); ("msg_id", `Int msg_id) ]
  | Topology { msg_id; topology } ->
      `Assoc
        [
...
type outbound_msg = { src : string; dest : string; body : outbound_body }
[@@deriving yojson]

let type_of_inbound_msg (inbound_msg : inbound_msg) =
  inbound_body_id inbound_msg.body
