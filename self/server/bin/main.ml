...
type state = {
  ...
  callbacks : (int, request_body Eio.Promise.u) Hashtbl.t;
}
  ...
  while not (StringSet.is_empty !unacked) do
    ...
    StringSet.iter (fun dest ->
      ...
      let req = {
        ...
        src = state.node_id;
        dest;
        body = BroadcastBody { message; msg_id = Some msg_id }
      } in
      let promise, resolver = Eio.Promise.create () in
      Eio.Mutex.use_rw ~protect:true state_mutex (fun () ->
        Hashtbl.replace state.callbacks msg_id resolver
      );
      let req_str = yojson_of_request req |> Yojson.Safe.to_string in
      ...
      Eio.Fiber.fork ~sw (fun () ->
        ...
        | BroadcastOk _ ->
            Eio.traceln "Got response from %s" dest;
            Eio.Mutex.use_rw ~protect:true state_mutex (fun () ->
    ...
    Eio.Time.sleep (Eio.Stdenv.clock Eio.Stdenv.default) 1.0
  done;
          ...
          let handled_by_callback =
            ...
            | Some msg_id ->
                Eio.Mutex.use_rw ~protect:true state_mutex (fun () ->
                  ...
                  | Some resolver ->
                      Eio.Promise.resolve resolver req.body;
...
let send_request_with_callback ~state ~state_mutex ~msg_id ~request =
  let promise, resolver = Eio.Promise.create () in
  Eio.Mutex.use_rw ~protect:true state_mutex (fun () ->
    Hashtbl.replace state.callbacks msg_id resolver;
