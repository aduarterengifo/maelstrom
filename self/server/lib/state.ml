...
type locks = { state : Eio.Mutex.t; stdout : Eio.Mutex.t; stderr : Eio.Mutex.t }
...
type 'a state = {
  ...
  callbacks : (int, Better_msg.peer_msg -> unit) Hashtbl.t;
  env : 'a env;
  locks : locks;
}

let next_msg_id (state : _ state) = Atomic.fetch_and_add state.msg_id 1
